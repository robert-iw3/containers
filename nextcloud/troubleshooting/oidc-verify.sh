...
CID="$(env_get OIDC_NEXTCLOUD_CLIENT_ID)"; CID="${CID:-nextcloud}"
CSECRET="$(env_get OIDC_NEXTCLOUD_CLIENT_SECRET)"
REDIR="$(env_get NEXTCLOUD_OIDC_REDIRECT_URI)"
...
if [ -n "$CSECRET" ]; then
  ...
  warn "OIDC_NEXTCLOUD_CLIENT_SECRET not in .env — skipping token-endpoint check"
fi
