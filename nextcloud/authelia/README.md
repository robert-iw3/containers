# Authelia SSO (nextcloud `sso` profile)

Authelia acts as an **OpenID Connect provider** for Nextcloud (real SSO for the
web UI via the `user_oidc` app; desktop/mobile/DAV clients keep working with
...
## Bootstrap (also see ../nextcloud-configure-oidc.sh)

```sh
cd nextcloud/authelia && mkdir -p secrets
...
# OIDC client secret: keep the plaintext for Nextcloud, store only the hash here
...
Nextcloud — `../nextcloud-configure-oidc.sh` automates the Nextcloud side.
