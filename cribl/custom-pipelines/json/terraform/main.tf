# Provision the JSON pipeline + file collector on a Cribl leader via its
...
resource "null_resource" "create_collector" {
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      ...
      [ -n "$TOKEN" ] || { echo "Cribl API login failed"; exit 1; }
      AUTH_HEADER="Authorization: Bearer $TOKEN"
      EXISTS=$(curl -s -o /dev/null -w "%%{http_code}" -H "$AUTH_HEADER" "$CRIBL_HOST/api/v1/m/$GROUP/lib/jobs/json_file_collector")
      if [ "$EXISTS" = "200" ]; then
        echo "Collector json_file_collector exists; skipping."
        exit 0
      fi
      PAYLOAD=$(cat <<JSON
      {
        "id": "json_file_collector",
        "type": "collection",
        "ttl": "4h",
        "removeFields": [],
        "resumeOnBoot": false,
