...
try:
    ...
    MAIN_OUTPUT = config.get('json', 'main_output', fallback='default')
    METRICS_OUTPUT = config.get('json', 'metrics_output', fallback='devnull')
...
collector_payload = {
    ...
    'collector': {'type': 'filesystem', 'conf': {'path': JSON_DIR, 'extractors': [], 'filenames': [FILE_FILTER]}},
    'input': {
