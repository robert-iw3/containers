#!/bin/bash
# Verify the xml pipeline exists on the leader (group-scoped, variant-suffixed).
set -euo pipefail
...
PIPELINE_GROUP=$(parse_ini "xml" "pipeline_group")
PIPELINE_GROUP=${PIPELINE_GROUP:-default}
PIPELINE_VARIANT=$(parse_ini "xml" "pipeline_variant")
