# Provision the CSV pipeline + file collector on a Cribl leader via its
# REST API (same endpoints as the bash/ and python/ methods). Uses only
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      ...
      sed -e "s/{{pipeline_id}}/$PIPELINE_ID_VARIANT/g" \
          ...
          -e "s/{{delimiter}}/$DELIMITER/g" \
          -e "s/{{has_header}}/$HAS_HEADER/g" \
          "$JSON_FILE" > "$TMP"
...
resource "null_resource" "create_collector" {
  ...
  provisioner "local-exec" {
    ...
    environment = {
      ...
      GROUP               = var.pipeline_group
      PIPELINE_ID_VARIANT = local.pipeline_id_variant
      COLLECT_DIR         = var.csv_dir
      FILE_FILTER         = var.file_filter
      TRACKING_FIELD      = var.tracking_field
    }
    command = <<-EOT
      set -euo pipefail
      TOKEN=$(curl -s -X POST "$CRIBL_HOST/api/v1/auth/login" -H 'Content-Type: application/json' -d "{\"username\":\"$CRIBL_USER\",\"password\":\"$CRIBL_PASS\"}" | sed -n 's/.*"token":"\([^"]*\)".*/\1/p')
      [ -n "$TOKEN" ] || { echo "Cribl API login failed"; exit 1; }
      AUTH_HEADER="Authorization: Bearer $TOKEN"
      EXISTS=$(curl -s -o /dev/null -w "%%{http_code}" -H "$AUTH_HEADER" "$CRIBL_HOST/api/v1/m/$GROUP/lib/jobs/csv_file_collector")
      if [ "$EXISTS" = "200" ]; then
        echo "Collector csv_file_collector exists; skipping."
        exit 0
      fi
      PAYLOAD=$(cat <<JSON
      {
        "id": "csv_file_collector",
        "type": "collection",
        "ttl": "4h",
        "removeFields": [],
        "resumeOnBoot": false,
        ...
        "collector": {"type": "filesystem", "conf": {"path": "$COLLECT_DIR", "extractors": [], "filenames": ["$FILE_FILTER"]}},
