#!/bin/bash
# Verify the csv pipeline exists on the leader (group-scoped, variant-suffixed).
set -euo pipefail
...
PIPELINE_GROUP=$(parse_ini "csv" "pipeline_group")
PIPELINE_GROUP=${PIPELINE_GROUP:-default}
PIPELINE_VARIANT=$(parse_ini "csv" "pipeline_variant")
